...
public sealed class CustomFieldTools(IEasyVereinApiClient client)
{
    ...
    public async Task<string> ListCustomFields(
        ...
        [Description("Kind-code filter, lowercase (e.g. 'e','h','j','i')")] string? kind,
        [Description("Field-type filter, lowercase (e.g. 't','z','d','c','s')")] string? settingsType,
    ...
    public async Task<string> CreateCustomField(
        ...
        [Description("Field type code, lowercase: 't' text, 'f' multiline, 'z' decimal, 'd' date, 'c' checkbox, 'r' date-range, 's' select, 'a' multi-select, 'b' file, 'm' reminder (default 't'). Note: the API rejects capitalized values.")] string? settingsType,
        [Description("Kind code, lowercase (custom fields: 'e'=members, 'h'=events, 'j'=contact-details, 'i'=inventory). Note: the API rejects capitalized values.")] string? kind,
